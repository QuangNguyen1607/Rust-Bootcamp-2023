...
// Output: &str
// Noted by learner:
// change u32 -> i32 because in unit test, teacher was passed value of -5, which is negative value. So i have to change u32 -> i32
...
fn is_prime(n: u32) -> bool {
    ...
    let limit = (n as f64).sqrt() as u32; // Why use this line ? What is f64 => use to convert number to a 64 bit floating-point number. why we have to f64 because sqrt() return floating number. as u32 convert floating number to integers
    for i in 2..=limit {
        if n % i == 0 {
            return false;
        }
